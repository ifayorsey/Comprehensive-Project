...
Figure **x** is a simple decision tree that fits chooses petal length and width as nodes.  In this instance, all observations in the test set were correctly guessed. 

Do to their high variance, resulting trees can vary greatly if the training data is modified, and can lead to drastically different test labels. Traditionally, classification and regression decision trees are associated with high variance and low bias.
...
knitr::kable(table(tree_pred, minitest$Species))

rpart.plot::rpart.plot(tree, box.palette="RdBu", shadow.col="gray")
